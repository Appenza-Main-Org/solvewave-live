#!/usr/bin/env python3
"""Generate SolveWave architecture-diagram.png using PIL."""
...
# -- Title --
draw_text(80, 40, "SOLVEWAVE", font=font_title, fill=C_EMERALD)
draw_text(480, 50, "— Architecture Overview (v0.6.0)", font=font_h3, fill=C_SLATE_500)
draw_text(80, 92, "See it. Say it. Solve it.  |  Gemini Live Agent Challenge", font=font_small, fill=C_SLATE_500)
...
UI_Y = BROWSER_Y + 95
draw_rounded_rect((100, UI_Y, W-100, UI_Y+640), fill="#0a1628", outline=C_SLATE_700, radius=16, width=1)
...
# -- Hooks Section (updated with WebRTC) --
HOOKS_Y = UI_Y + 680
draw_rounded_rect((100, HOOKS_Y, W-100, HOOKS_Y+290), fill="#0a1628", outline=C_SLATE_700, radius=16, width=1)
draw_text(120, HOOKS_Y+12, "Core Hooks & Audio Transport", font=font_h3, fill=C_SLATE_400)

# useSessionSocket
draw_rounded_rect((130, HOOKS_Y+50, 830, HOOKS_Y+155), fill=C_SLATE_800, outline=C_SKY, radius=12, width=1)
draw_text(150, HOOKS_Y+58, "useSessionSocket.ts", font=font_body, fill=C_SKY)
draw_text(150, HOOKS_Y+88, "WebSocket control channel  |  Live state (9 states)", font=font_small, fill=C_SLATE_400)
draw_text(150, HOOKS_Y+110, "send: text, image  |  WebRTC signaling (SDP)", font=font_small, fill=C_SLATE_400)
draw_text(150, HOOKS_Y+132, "WS binary fallback when WebRTC unavailable", font=font_small, fill=C_SLATE_400)

# useWebRTC
draw_rounded_rect((860, HOOKS_Y+50, 1560, HOOKS_Y+155), fill=C_SLATE_800, outline=C_PURPLE, radius=12, width=1)
draw_text(880, HOOKS_Y+58, "useWebRTC.ts", font=font_body, fill=C_PURPLE)
draw_text(880, HOOKS_Y+88, "RTCPeerConnection (Opus codec)", font=font_small, fill=C_SLATE_400)
draw_text(880, HOOKS_Y+110, "getUserMedia: AEC + NS + AGC enabled", font=font_small, fill=C_SLATE_400)
draw_text(880, HOOKS_Y+132, "SDP offer/answer via WS  |  Mic mute/unmute", font=font_small, fill=C_SLATE_400)

# useSessionTimer (compact)
draw_rounded_rect((1590, HOOKS_Y+50, W-130, HOOKS_Y+155), fill=C_SLATE_800, outline=C_AMBER, radius=12, width=1)
draw_text(1610, HOOKS_Y+58, "useSessionTimer", font=font_body, fill=C_AMBER)
draw_text(1610, HOOKS_Y+88, "mm:ss display", font=font_small, fill=C_SLATE_400)
draw_text(1610, HOOKS_Y+110, "Start on connect", font=font_small, fill=C_SLATE_400)
draw_text(1610, HOOKS_Y+132, "Freeze on stop", font=font_small, fill=C_SLATE_400)

# useVoiceTranscription
draw_rounded_rect((130, HOOKS_Y+170, 830, HOOKS_Y+275), fill=C_SLATE_800, outline=C_ROSE, radius=12, width=1)
draw_text(150, HOOKS_Y+178, "useVoiceTranscription.ts", font=font_body, fill=C_ROSE)
draw_text(150, HOOKS_Y+208, "Web Speech API (browser-native)", font=font_small, fill=C_SLATE_400)
draw_text(150, HOOKS_Y+230, "Partial -> update in-place  |  Final -> sendTextQuiet()", font=font_small, fill=C_SLATE_400)
draw_text(150, HOOKS_Y+252, "Echo suppression (skip if isSpeaking)", font=font_small, fill=C_SLATE_500)

# Transport comparison box
draw_rounded_rect((860, HOOKS_Y+170, W-130, HOOKS_Y+275), fill=C_SLATE_800, outline=C_ORANGE, radius=12, width=1)
draw_text(880, HOOKS_Y+178, "Audio Transport", font=font_body, fill=C_ORANGE)
draw_text(880, HOOKS_Y+208, "PRIMARY: WebRTC (Opus/DTLS/SRTP over UDP)", font=font_small, fill=C_EMERALD)
draw_text(880, HOOKS_Y+230, "FALLBACK: WebSocket binary (PCM 16kHz/24kHz)", font=font_small, fill=C_SLATE_400)
draw_text(880, HOOKS_Y+252, "Both feed same asyncio.Queue on backend", font=font_small, fill=C_SLATE_500)

# -- Arrow: Browser -> Backend (dual path) --
ARROW1_Y = BROWSER_Y + BROWSER_H
# WebRTC arrow (left)
draw_arrow(W//2 - 200, ARROW1_Y, W//2 - 200, ARROW1_Y+100, color=C_PURPLE, width=4)
draw_text(W//2 - 180, ARROW1_Y+15, "WebRTC: Opus audio (primary)", font=font_body, fill=C_PURPLE)
draw_text(W//2 - 180, ARROW1_Y+42, "DTLS/SRTP over UDP  |  AEC/NS/AGC", font=font_small, fill=C_SLATE_500)

# WebSocket arrow (right)
draw_arrow(W//2 + 200, ARROW1_Y, W//2 + 200, ARROW1_Y+100, color=C_EMERALD, width=4)
draw_text(W//2 + 220, ARROW1_Y+15, "WebSocket: /ws/session (control)", font=font_body, fill=C_EMERALD)
draw_text(W//2 + 220, ARROW1_Y+42, "JSON: text/image/signaling  |  Binary: PCM fallback", font=font_small, fill=C_SLATE_500)

# -- Layer 2: BACKEND --
BACKEND_Y = ARROW1_Y + 110
BACKEND_H = 950
draw_rounded_rect((60, BACKEND_Y, W-60, BACKEND_Y+BACKEND_H), fill=C_SLATE_900, outline=C_EMERALD, radius=24, width=3)
draw_text(80, BACKEND_Y+16, "BACKEND  --  FastAPI + asyncio + aiortc", font=font_h2, fill=C_EMERALD)
...
# session_manager.py + webrtc_handler.py
SM_Y = MP_Y + 85
draw_rounded_rect((100, SM_Y, W-100, SM_Y+280), fill="#0a1628", outline=C_SKY, radius=12, width=1)
draw_text(120, SM_Y+10, "session_manager.py -- WebSocket Lifecycle + WebRTC Signaling", font=font_h3, fill=C_SKY)
...
draw_text(150, SM_Y+86, "1. Accept WebSocket + send ICE servers", font=font_small, fill=C_SLATE_400)
draw_text(150, SM_Y+108, "2. Create asyncio.Queue (decouple receive/send)", font=font_small, fill=C_SLATE_400)
draw_text(150, SM_Y+130, "3. receive_loop() -- route text/image/signaling", font=font_small, fill=C_SLATE_400)
draw_text(150, SM_Y+152, "4. LiveClient.run() -- Queue -> Gemini -> Browser", font=font_small, fill=C_SLATE_400)
draw_text(150, SM_Y+178, "5. On disconnect: close WebRTC + build recap", font=font_small, fill=C_SLATE_400)

# WebRTC handler
draw_rounded_rect((130, SM_Y+225, 900, SM_Y+268), fill=C_SLATE_800, outline=C_PURPLE, radius=10)
draw_text(150, SM_Y+233, "webrtc_handler.py -- aiortc: Opus decode -> 16kHz PCM -> Queue  |  24kHz PCM -> 48kHz -> Opus encode", font=font_tiny, fill=C_PURPLE)

# Mode addendums
draw_rounded_rect((930, SM_Y+50, W-130, SM_Y+268), fill=C_SLATE_800, outline=C_ORANGE, radius=10)
...
draw_text(950, SM_Y+205, "WebRTC signaling:", font=font_body, fill=C_PURPLE)
draw_text(950, SM_Y+232, '"rtc_offer" -> handle_offer() -> rtc_answer', font=font_small, fill=C_SLATE_400)
...
draw_text(150, LC_Y+108, "downstream: Gemini -> WebRTC/WS", font=font_small, fill=C_SLATE_400)
draw_text(150, LC_Y+130, "Barge-in detection -> interrupt signal", font=font_small, fill=C_SLATE_400)
...
draw_text(120, TA_Y+10, "tutor_agent.py -- SolveWave Persona & Tools", font=font_h3, fill=C_EMERALD)
...
# Version footer
draw_text(80, LEG_Y+50, "SolveWave v0.6.0  |  2026-03-15  |  Gemini Live Agent Challenge", font=font_small, fill=C_SLATE_500)

# -- Crop to content (remove bottom whitespace) --
crop_bottom = LEG_Y + 80
